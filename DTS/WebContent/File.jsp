										...
										<td class="left">
						   					<select name="bug_id" id="bug_id" data-validate="validate(otherThanNone(none))">
									    		<option value="none">---Select Bug---</option>
									     		<sql:setDataSource var="dataSource" driver="com.mysql.cj.jdbc.Driver" url="jdbc:mysql://localhost/dts" user="root" password="root" scope="session"/>
...
<h2>All Files</h2>
				...
				<c:when test="${sessionScope.admin_yes==1 }">
		      	...
		      	<c:otherwise>
					...
					<sql:setDataSource var="dataSource" driver="com.mysql.cj.jdbc.Driver" url="jdbc:mysql://localhost/dts" user="root" password="root" scope="session"/>
					<sql:query var="qry" dataSource="${dataSource}">select DISTINCT file_id, file_name, file_size, file_desc, bug_id  from dts_file where bug_id in(select bug_id from dts_bug where bug_from='${sessionScope.user_logged}')</sql:query>
